...
def clean_field(value):
    """Chuyển nan, None, null -> chuỗi rỗng"""
...
def generate_restaurant_info(
    ...
    df["Address_new"] = df["norm_loc"].apply(lambda n: get_cached(n, "Address"))
    df["Description_new"] = df["norm_loc"].apply(lambda n: get_cached(n, "Description"))
    df["LongDesc_new"] = df["norm_loc"].apply(lambda n: get_cached(n, "Long Description"))

    # Giữ nguyên tất cả các cột gốc, chỉ ghi đè 3 cột bị thiếu
    df["Address"] = df["Address_new"].combine_first(df["Address"])
    df["Description"] = df["Description_new"].combine_first(df["Description"])
    df["Long Description"] = df["LongDesc_new"].combine_first(df["Long Description"])

    df.drop(columns=["Address_new", "Description_new", "LongDesc_new", "norm_loc"], inplace=True)
